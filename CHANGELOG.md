...
- **JSON 请求体插入图片 Base64**：在 JSON 请求体面板中支持填写字段路径（点号嵌套、数组下标）、选择 Data URL 或纯 Base64，通过扩展侧文件对话框选择图片并写入目标字段。对大体积编码使用占位符在编辑器中展示，发送请求与保存接口时自动展开为完整内容；加载已保存的大段图片 Base64 时也会自动折叠为占位符，减轻编辑器卡顿与「完整请求」预览刷屏。
...
>>>>>>> 5963f2d22b4733496b0b2db521a6381344199d48
